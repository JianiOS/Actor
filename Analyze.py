...
def analyze_list_path(callback, path=""):
    ...
    for item in items:
        ...
        def detail_callback(state,download_urls=[], actors=[]):
            "Detail Call Back"
            if state == download_state.error:
                a_movie.state = download_state.error
            else:
                a_movie.download_urls = download_urls
                a_movie.state = download_state.success

        print("Analyze Detail Path {}".format(href))
        analyze_detail_path(detail_callback, path=href)

        if a_movie.state == download_state.success:
            print(a_movie)
        else:
            print("Movie Analyze Failed {}".format(href))
...
def analyze_detail_path(callback, path=""):
    ...
    callback(download_state.success, download_urls=urls)
